...
-- Suppression de la prémiere table edusign 
--DROP TABLE edusign;
...
-- 4. Sélectionner toutes les lignes de la table edusign ordonnées par date de signature de la plus récente à la plus ancienne et par user_id ascendant
 ...
 -- Compter le nombre de ligne au sein de la table edusign dont le prénom est Bella, le nom de la colonne finale doit être volume
      --SELECT COUNT(*) AS [volume] FROM edusign WHERE user_id = 1;
--SELECT * FROM edusign;
-- SELECT created_at, COUNT(*) AS [e nombre d’apprenantes] FROM edusign GROUP BY created_at;
-- UPDATE edusign SET user_id=1 WHERE id=1;
-- UPDATE edusign SET user_id=3 WHERE id=2;
-- UPDATE edusign SET user_id=1 WHERE id=3;
-- UPDATE edusign SET user_id=4 WHERE id=4;
